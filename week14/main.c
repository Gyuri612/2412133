...
// ----- EX. 4 : player ------------
        if (player_status[turn] != PLAYERSTATUS_LIVE)	// die or end
        {
...
// ----- EX. 4 : player ------------
		...
		if((N_BOARD-player_position[turn])>dieResult)
    	...
    	if (coinResult>0)
    	printf("player %s get %i coin!\n",player_name[turn],coinResult);
        
        //step 2-5. end process
        
        if (player_position[turn] == N_BOARD - 1){	
        	player_status[turn] = PLAYERSTATUS_END;
    		printf("Player %s has reached the end!\n", player_name[turn]);
		}
